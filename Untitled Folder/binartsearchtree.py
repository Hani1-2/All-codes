...
##        return root.data
##    p = root
##    while p.lc is not None:
##        p = p.lc
##    return p.data
##another way to code the minimum function is this:

def minimum(root):
    if root is None: return None
    if root.lc is None:
        return root.data
    return minimum(root.lc)

def maximum(root):
    if root is None: return None
    if root.rc is None:
        return root.data
    return maximum(root.rc)
...
#v=[8,3,10,1,6,14,4,7,13]
v=[54,16,39,46,72,80,61,64,12,45,74]
root=build_bst(v)
delfrombst(root,61)
root.traversebf()
print("Sorted list ")
print(getsortedlist(root))
mini = minimum(root)
##x=searchbst(root,6)
##print(x)
print("The minimum no in given BST is: ",mini)
maxi = maximum(root)
print("The maximum no in given BST is: ",maxi)
