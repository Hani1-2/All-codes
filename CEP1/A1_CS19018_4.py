...
from A1_CS19018_1 import *#importing the librarian class for further usage
from A1_CS19018_2 import *#after importing librarian as a module we import all the funtionality of the librarian class
from A1_CS19018_3 import *#importing all the funcutionality og gui module
from A1_CS19018_5 import *
...
class main_screen(Tk):
    ...
    def __init__(self,id=0):
        ...
        self.title("LIBRARY MANAGEMENT SYSTEM")#used to put the title on the top
        Label(self,text="LIBRARY MANAGEMENT SYSTEM", bg='pink', width="300", height="3", font=("Calibri", 13)).pack()#setting the width,height,font in which the title is printed and placing it on the screen using pack
        self.config(bg='cadet blue')#setting the background color
        Label(self,text="").pack()
        Button(self,text="USER", height="3", width="30",command=self.second_screen).pack()#placing a button named user on the screen,second screen here creates a second screen on the interface
        Label(self,text="").pack()
        Label(self,text="").pack()
        Button(self,text="LIBRARIAN", height="3", width="30",command=Lib_screen).pack()#used to place the librarian botton on the screen

        self.mainloop()#an infinite loop to run the gui till called off


    def second_screen(self):#creating a screen after the librarian/user screen that asks the user to login or register
    ...
    def login(self):#used to ask the user to enter the credensitials to login
        ...
        self.username_verify = StringVar()#used to verify the entered username by comparing it with names in the log
        self.password_verify = StringVar()#used to verify the eentered password by comparing it with password in the log
        self.id = StringVar()#used to identify the entered id by comparing it with password in the log
    ...
    def login_sucess(self):#displays a login successful screen after the correct info has been entered
    ...
    def password_not_recognised(self):#displays a screen that tells the user if the entered password is wrong
    ...
    def login_verify(self):#verifing the password and username
        ...
        with open('user_login','r') as f:#opening the file of user log
            ...
            if (id1+','+username1+' '+password1)in value:#this line of the code will run if the id,password and username are correct
    ...
    def register_user(self):#used to register a user
        ...
        main_screen.id+=1
        file = open('user_login', "a+")#this line of code is used to open the log file
